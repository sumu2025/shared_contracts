...
class ToolParameter(BaseModel):
    ...
    def validate_default_type(self) -> 'ToolParameter':
        ...
        if self.default is not None:
            ...
            elif self.type == ParameterType.INTEGER and not isinstance(self.default, int):
                raise ValueError(f"默认值类型应为整数，而不是 {type(self.default)}")
            elif self.type == ParameterType.NUMBER and not isinstance(self.default, (int, float)):
                raise ValueError(f"默认值类型应为数字，而不是 {type(self.default)}")
            elif self.type == ParameterType.BOOLEAN and not isinstance(self.default, bool):
                raise ValueError(f"默认值类型应为布尔值，而不是 {type(self.default)}")
            elif self.type == ParameterType.ARRAY and not isinstance(self.default, list):
                raise ValueError(f"默认值类型应为数组，而不是 {type(self.default)}")
            elif self.type == ParameterType.OBJECT and not isinstance(self.default, dict):
...
class ToolDefinition(BaseModel):
    ...
    returns: Optional[ToolParameter] = Field(
        default=None,
        description="返回值描述"
    )
