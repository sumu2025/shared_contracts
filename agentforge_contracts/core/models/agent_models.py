...
class AgentConfig(BaseModel):
    ...
    description: Optional[str] = Field(
        default=None, max_length=1000, description="代理描述"
    )

    agent_type: AgentType = Field(..., description="代理类型")

    base_model_id: str = Field(..., min_length=1, description="代理使用的基础模型ID")

    system_prompt: str = Field(..., min_length=1, description="系统提示词")

    tools: List[str] = Field(default_factory=list, description="代理可用工具列表")
    ...
    capabilities: Set[str] = Field(default_factory=set, description="代理能力标签")
...
class AgentState(BaseModel):
    ...
    status: str = Field(..., description="代理当前状态")

    current_task: Optional[str] = Field(default=None, description="当前正在执行的任务")

    last_active: datetime = Field(default_factory=datetime.utcnow, description="上次活动时间")

    conversation_id: Optional[UUID] = Field(default=None, description="当前对话ID")

    memory_snapshot: Dict[str, Any] = Field(default_factory=dict, description="代理记忆快照")

    execution_metrics: Dict[str, Any] = Field(default_factory=dict, description="执行指标")
    ...
    def validate_status(cls, value: str) -> str:
        ...
        if value not in valid_statuses:
            raise ValueError(
                f"无效的状态: {value}. " f"有效的状态包括: {', '.join(valid_statuses)}"
            )
