        ...
        try:
            ...
            if config.model_type == ModelType.CHAT:
                ...
                payload = {
                    ...
                    or config.capabilities.get("temperature", 0.7),
                    "max_tokens": max_tokens
                    or config.capabilities.get("max_tokens", 1000),
                }
            else:
            ...
            with track_performance(
                "openai_api_call", ServiceComponent.MODEL_SERVICE
            ) as span:
            ...
            return BaseResponse(
                request_id=request_id, success=True, data=model_response
            )
    ...
    async def generate_completion(
        ...
        **options: Any,
        ...
        try:
            ...
            # 调用API并测量性能
            with track_performance(
                "claude_api_call", ServiceComponent.MODEL_SERVICE
            ) as span:
                response_data = await self._call_claude_api(
                    payload=payload, api_key=api_key, timeout=config.timeout
                )
