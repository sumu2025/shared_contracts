...
class SimpleModelService(ModelServiceInterface):
    ...
    async def get_model(self, model_id: str) -> BaseResponse[ModelConfig]:
        ...
        if model_id not in self.models:
            return BaseResponse(
                request_id=uuid.uuid4(), success=False, error="Model not found"
            )
    ...
    async def generate(
        self, model_id: str, prompt: str, parameters: Dict[str, Any] = None
        ...
        return BaseResponse(request_id=uuid.uuid4(), success=True, data=model_response)

    @with_monitoring(component=ServiceComponent.MODEL_SERVICE)
    async def stream_generate(
        self, model_id: str, prompt: str, parameters: Dict[str, Any] = None
    ) -> AsyncIterable[BaseResponse[ModelResponse]]:
        """流式生成模型响应。"""
        if model_id not in self.models:
            yield BaseResponse(
                request_id=uuid.uuid4(), success=False, error="Model not found"
        ...
        for part in response_parts:
            ...
            yield BaseResponse(
                request_id=uuid.uuid4(), success=True, data=model_response
            )
...
class SimpleToolService(ToolServiceInterface):
    ...
    async def register_tool(self, tool: ToolDefinition) -> BaseResponse[ToolDefinition]:
        ...
        return BaseResponse(request_id=uuid.uuid4(), success=True, data=tool)
    ...
    async def get_tool(self, tool_id: str) -> BaseResponse[ToolDefinition]:
        ...
        if tool_id not in self.tools:
            return BaseResponse(
                request_id=uuid.uuid4(), success=False, error="Tool not found"
            )
    ...
    async def execute_tool(
        ...
        if not validation_result.valid:
            ...
            return BaseResponse(
                request_id=uuid.uuid4(), success=False, error=error_message
            )
        ...
        tool_result = ToolResult(
            tool_id=tool_id, status=status, result=result, created_at=datetime.now()
        )
        ...
        return BaseResponse(request_id=uuid.uuid4(), success=True, data=tool_result)
...
class SimpleAgentService(AgentServiceInterface):
    ...
    def __init__(
        self, model_service: ModelServiceInterface, tool_service: ToolServiceInterface
    ):
        self.agents = {}  # 存储代理配置
    ...
    async def get_agent(self, agent_id: str) -> BaseResponse[AgentConfig]:
        ...
        if agent_id not in self.agents:
            return BaseResponse(
                request_id=uuid.uuid4(), success=False, error="Agent not found"
            )
    ...
    async def get_agent_state(self, agent_id: str) -> BaseResponse[AgentState]:
        ...
        if agent_id not in self.agent_states:
            return BaseResponse(
                request_id=uuid.uuid4(), success=False, error="Agent state not found"
            )

        return BaseResponse(
            request_id=uuid.uuid4(), success=True, data=self.agent_states[agent_id]
        )
    ...
    async def process_message(self, agent_id: str, message: str) -> BaseResponse[str]:
        ...
        if tool_match:
            ...
            tool_result = await self.tool_service.execute_tool(
                tool_match.tool_id, parameters
            )

            if tool_result.success:
                # 在响应中加入工具执行结果
                tool_response = (
                    f"我使用{tool_match.tool_id}工具获取到的结果是: {tool_result.data.result}"
                )
            ...
            # 直接使用模型生成响应
            model_response = await self.model_service.generate(config.model_id, message)
        ...
        return BaseResponse(request_id=uuid.uuid4(), success=True, data=response)

    @with_monitoring(component=ServiceComponent.AGENT_SERVICE)
    async def stream_process_message(
        self, agent_id: str, message: str
    ) -> AsyncIterable[BaseResponse[str]]:
        """流式处理用户消息。"""
        if agent_id not in self.agents:
            yield BaseResponse(
                request_id=uuid.uuid4(), success=False, error="Agent not found"
        ...
        async for chunk_response in self.model_service.stream_generate(
            config.model_id, message
        ):
            if chunk_response.success:
                chunk = chunk_response.data.response
                full_response += chunk

                yield BaseResponse(request_id=uuid.uuid4(), success=True, data=chunk)
            else:
                yield BaseResponse(
...
async def startup_event():
    ...
    await agent_service.register_agent(
        AgentConfig(
            ...
            parameters={"tool_usage_threshold": 0.7, "max_tool_uses": 3},
        )
    )
...
async def chat(request: Request):
    ...
    try:
        ...
        if not message:
            return JSONResponse(
                status_code=400, content={"success": False, "error": "消息不能为空"}
            )
        ...
        if response.success:
            ...
            return {"success": True, "session_id": session_id, "message": response.data}
        else:
            return JSONResponse(
                status_code=500, content={"success": False, "error": response.error}
            )

    except Exception as e:
        logger.error(f"处理聊天请求时出错: {str(e)}")
        return JSONResponse(
            status_code=500, content={"success": False, "error": f"内部服务器错误: {str(e)}"}
        )
    ...
    try:
        ...
        while True:
            ...
            if message_type == "message":
                ...
                # 流式处理消息
                full_response = ""
                async for chunk_response in agent_service.stream_process_message(
                    ...
                    if chunk_response.success:
                        ...
                        await websocket.send_json(
                            {"type": "error", "error": chunk_response.error}
                        )
                        break
...
async def run_example_workflow():
    ...
    weather_response = await agent_service.process_message(
        "chat-assistant", weather_message
    )
